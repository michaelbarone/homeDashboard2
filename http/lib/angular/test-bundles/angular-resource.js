  ...
  describe("resource", function () {
    ...
    describe("basic usage", function () {
      ...
      it("should build resource with default param", function () {
        ...
        var LineItem = $resource("/Order/:orderId/Line/:id:verb", { orderId: "123", id: "@id.key", verb: ".visa", minimum: 0.05 });
      ...
      it("should build resource with action default param overriding default param", function () {
        ...
        var TypeItem = $resource("/:type/:typeId", { type: "Order" }, { get: { method: "GET", params: { type: "Customer" } } });
      ...
      it("should update resource", function () {
        $httpBackend.expect("POST", "/CreditCard/123", '{"id":{"key":123},"name":"misko"}').respond({ id: { key: 123 }, name: "rama" });
      ...
      it("should post charge verb on instance", function () {
        $httpBackend.expect("POST", "/CreditCard/123!charge?amount=10", '{"id":{"key":123},"name":"misko"}').respond({ success: "ok" });
    ...
    describe("errors", function () {
      ...
      it("should fail if action expects an object but response is an array", function () {
        ...
        expect(failureSpy.calls.mostRecent().args[0]).toEqualMinErr(
          "$resource",
          "badcfg",
          "Error in resource configuration for action `query`. " + "Expected response to contain an array but got an object (Request: GET /Customer/123)"
        );
      });
      ...
      it("should fail if action expects an array but response is an object", function () {
        ...
        expect(failureSpy.calls.mostRecent().args[0]).toEqualMinErr(
          "$resource",
          "badcfg",
          "Error in resource configuration for action `get`. " + "Expected response to contain an object but got an array (Request: GET /Customer/123)"
        );
      });
    });
    ...
    describe("cancelling requests", function () {
      ...
      it("should use `cancellable` value if passed a non-numeric `timeout` in an action", inject(function ($log, $q, $rootScope) {
        spyOn($log, "debug");
        $httpBackend.whenGET("/CreditCard").respond({});
