 ...
 */
  ...
  describe("ngMessages", function () {
    ...
    it("should render the same message if multiple message keys match", inject(function ($rootScope, $compile) {
      element = $compile('<div ng-messages="col">' + '  <div ng-message="one, two, three">Message is set</div>' + "</div>")($rootScope);
      $rootScope.$digest();
    ...
    it("should use the when attribute when an element directive is used", inject(function ($rootScope, $compile) {
      element = $compile('<ng-messages for="col">' + '  <ng-message when="val">Message is set</div>' + "</ng-messages>")($rootScope);
      $rootScope.$digest();
    ...
    it("should render the same message if multiple message keys match based on the when attribute", inject(function ($rootScope, $compile) {
      element = $compile('<ng-messages for="col">' + '  <ng-message when=" one two three ">Message is set</div>' + "</ng-messages>")($rootScope);
      $rootScope.$digest();
    ...
    it("should allow a dynamic expression to be set when ng-message-exp is used", inject(function ($rootScope, $compile) {
      element = $compile('<div ng-messages="col">' + '  <div ng-message-exp="variable">Message is crazy</div>' + "</div>")($rootScope);
      $rootScope.$digest();
    ...
    it("should allow a dynamic expression to be set when the when-exp attribute is used", inject(function ($rootScope, $compile) {
      element = $compile('<ng-messages for="col">' + '  <ng-message when-exp="variable">Message is crazy</ng-message>' + "</ng-messages>")($rootScope);
      $rootScope.$digest();
    ...
    they("should insert and remove matching inner elements when $prop is used as a value", { true: true, 1: 1, "{}": {}, "[]": [], "[null]": [null] }, function (prop) {
      inject(function ($rootScope, $compile) {
        element = $compile('<div ng-messages="col">' + '  <div ng-message="blue">This message is blue</div>' + '  <div ng-message="red">This message is red</div>' + "</div>")(
          $rootScope
        );
    ...
    it("should add ng-active/ng-inactive CSS classes to the element when errors are/aren't displayed", inject(function ($rootScope, $compile) {
      element = $compile('<div ng-messages="col">' + '  <div ng-message="ready">This message is ready</div>' + "</div>")($rootScope);
    ...
    it("should automatically re-render the messages when other directives dynamically change them", inject(function ($rootScope, $compile) {
      element = $compile(
        '<div ng-messages="col">' +
    ...
    // issue #12856
    it("should only detach the message object that is associated with the message node being removed", inject(function ($rootScope, $compile, $animate) {
    ...
    describe("ngMessage nested nested inside elements", function () {
      it("should not crash or leak memory when the messages are transcluded, the first message is " + "visible, and ngMessages is removed by ngIf", function () {
        ...
        inject(function ($rootScope, $compile) {
          element = $compile(
            '<div><div ng-if="show"><div message-wrap col="col">' + '        <div ng-message="a">A</div>' + '        <div ng-message="b">B</div>' + "</div></div></div>"
          )($rootScope);
      ...
      it("should not crash when the first of two nested messages is removed", function () {
        inject(function ($rootScope, $compile) {
          element = $compile(
            '<div ng-messages="col">' + '<div class="wrapper">' + '<div remove-me ng-message="a">A</div>' + '<div ng-message="b">B</div>' + "</div>" + "</div>"
          )($rootScope);
    ...
    describe("default message", function () {
      it("should render a default message when no message matches", inject(function ($rootScope, $compile) {
        element = $compile('<div ng-messages="col">' + '  <div ng-message="val">Message is set</div>' + "  <div ng-message-default>Default message is set</div>" + "</div>")(
          $rootScope
        );
        $rootScope.$apply(function () {
      ...
      it("should not render a default message with ng-messages-multiple if another error matches", inject(function ($rootScope, $compile) {
        element = $compile(
          '<div ng-messages="col" ng-messages-multiple>' +
      ...
      it("should handle a default message with ngIf", inject(function ($rootScope, $compile) {
        element = $compile(
          '<div ng-messages="col">' + '  <div ng-message="val">Message is set</div>' + '  <div ng-if="default" ng-message-default>Default message is set</div>' + "</div>"
        )($rootScope);
    ...
    describe("when including templates", function () {
      they(
        "should work with a dynamic collection model which is managed by ngRepeat",
        {
          ...
          '<ng-messages-include src="...">': '<ng-messages for="item">' + '<ng-messages-include src="abc.html"></ng-messages-include>' + "</ng-messages>"
        },
        function (html) {
          inject(function ($compile, $rootScope, $templateCache) {
            $templateCache.put("abc.html", '<div ng-message="a">A</div>' + '<div ng-message="b">B</div>' + '<div ng-message="c">C</div>');
      ...
      they(
        "should remove the $prop element and place a comment anchor node where it used to be",
        {
          ...
          '<ng-messages-include src="...">': '<ng-messages for="data">' + '<ng-messages-include src="abc.html"></ng-messages-include>' + "</ng-messages>"
        },
        function (html) {
          inject(function ($compile, $rootScope, $templateCache) {
      ...
      it("should cache the template after download", inject(function ($rootScope, $compile, $templateCache, $httpBackend) {
      ...
      it("should re-render the messages after download without an extra digest", inject(function ($rootScope, $compile, $httpBackend) {
        $httpBackend.expect("GET", "my-messages").respond(201, '<div ng-message="required">You did not enter a value</div>');

        element = $compile(
          '<div ng-messages="data">' + '  <div ng-messages-include="my-messages"></div>' + '  <div ng-message="failed">Your value is that of failure</div>' + "</div>"
        )($rootScope);
      ...
      it("should allow for overriding the remote template messages within the element depending on where the remote template is placed", inject(function (
        ...
        element = $compile(
          '<div ng-messages="data">' + '  <div ng-message="a">AAA</div>' + '  <div ng-messages-include="abc.html"></div>' + '  <div ng-message="c">CCC</div>' + "</div>"
        )($rootScope);
      ...
      it("should properly detect a previous message, even if it was registered later", inject(function ($compile, $rootScope, $templateCache) {
        $templateCache.put("include.html", '<div ng-message="a">A</div>');
        var html = '<div ng-messages="items">' + "<div ng-include=\"'include.html'\"></div>" + '<div ng-message="b">B</div>' + '<div ng-message="c">C</div>' + "</div>";
      ...
      it("should not throw if scope has been destroyed when template request is ready", inject(function ($rootScope, $httpBackend, $compile) {
        $httpBackend.expectGET("messages.html").respond('<div ng-message="a">A</div>');
        $rootScope.show = true;
        var html = '<div ng-if="show">' + '<div ng-messages="items">' + '<div ng-messages-include="messages.html"></div>' + "</div>" + "</div>";
      ...
      it("should not throw if the template is empty", inject(function ($compile, $rootScope, $templateCache) {
        var html = '<div ng-messages="items">' + '<div ng-messages-include="messages1.html"></div>' + '<div ng-messages-include="messages2.html"></div>' + "</div>";
    ...
    describe("when multiple", function () {
      they("should show all truthy messages when the $prop attr is present", { multiple: "multiple", "ng-messages-multiple": "ng-messages-multiple" }, function (prop) {
        inject(function ($rootScope, $compile) {
          element = $compile(
            '<div ng-messages="data" ' + prop + ">" + '  <div ng-message="one">1</div>' + '  <div ng-message="two">2</div>' + '  <div ng-message="three">3</div>' + "</div>"
          )($rootScope);
      ...
      it("should render all truthy messages from a remote template", inject(function ($rootScope, $compile, $templateCache) {
        $templateCache.put("xyz.html", '<div ng-message="x">X</div>' + '<div ng-message="y">Y</div>' + '<div ng-message="z">Z</div>');

        element = $compile('<div ng-messages="data" ng-messages-multiple="true">' + '<div ng-messages-include="xyz.html"></div>' + "</div>")($rootScope);
      ...
      it("should render and override all truthy messages from a remote template", inject(function ($rootScope, $compile, $templateCache) {
        $templateCache.put("xyz.html", '<div ng-message="x">X</div>' + '<div ng-message="y">Y</div>' + '<div ng-message="z">Z</div>');
